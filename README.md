...
- &lt;DotsMoveDelay /> - используем кастомный hook для создания задержки - эффекта "хвоста"
...
- &lt;FormMessage /> - отправка сообщения из формы, благодарность и возврат к чату по кнопке
- &lt;TodoListEffect /> - Трансформация данных без использования эффектов
- &lt;CatFriends /> - (useRef, useState) прокрутка к следующему img в слайдере при клике на кнопку
- &lt;VideoPlayer /> - использование useRef, useState для управления медиа-файлом
- &lt;LargeImages /> - оспользование useContext для передачи данных любому непрямому потомку
